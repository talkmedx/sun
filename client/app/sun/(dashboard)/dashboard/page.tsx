...
export default function DashboardPage() {
  ...
  const { data: summary, isLoading: summaryLoading } = useQuery({
    queryKey: ['dashboard-summary', bid],
    queryFn: async () => (await dashboardApi.summary(bid)).data.data,
  });

  const { data: charts, isLoading: chartsLoading } = useQuery({
    queryKey: ['dashboard-charts', bid],
    queryFn: async () => (await dashboardApi.charts(bid)).data.data,
  });
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Financial year {summary?.financial_year || '—'} overview
          </p>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          ...
          <CardContent>
            <ChartFrame loading={chartsLoading}>
